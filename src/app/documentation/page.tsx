    ...
    <>
      ...
      <main className="flex justify-between items-center flex-col gap-4 md:gap-2 md:flex-row h-[85%] pt-4 md:pt-8 mt-3 border-gray-500 border-t-2 border-dashed">
        <section className="w-[94%] md:w-[60%] h-[80vh] sm:h-[95vh] md:h-full mx-3 md:mx-4 shadow-2xl rounded-2xl border py-2 md:py-6 px-2 md:px-6 relative">
          <form action="" className="h-full w-full">
            <div className="flex items-center justify-between">
              <input
                ...
                onFocus={() =>  content.body.length > 2 ? setIsDisabled({ discardBtn: false, saveBtn: false }) : setIsDisabled({ discardBtn: true, saveBtn: true })}
                onMouseOut={() => content.title.length > 2 ? setIsDisabled({ discardBtn: false, saveBtn: false }) : setIsDisabled({ discardBtn: true, saveBtn: true })}
            ...
            <div className=" h-[58%] md:h-[78%]">
              <textarea
                ...
                onFocus={() => content.title.length > 2  ? setIsDisabled({ discardBtn: false, saveBtn: false }) : setIsDisabled({ discardBtn: true, saveBtn: true })}
                onMouseLeave={() => content.body.length > 2 ? setIsDisabled({ discardBtn: false, saveBtn: false }) : setIsDisabled({ discardBtn: true, saveBtn: true })}
            ...
            <div className=" flex justify-end mt-2.5 md:mt-4 text-xs md:text-[1rem]">
              {!isReadOnly && (
                <>
                  <button disabled={IsDisabled.discardBtn} className= {` ${IsDisabled.discardBtn ? `bg-gray-400 cursor-not-allowed ` : `bg-black hover:text-black hover:bg-white hover:border border-black cursor-pointer  `} text-white py-2  md:py-3 px-8 rounded-xl  `} onClick={(e) => {
                      e.preventDefault();
                  ...
                  <button
                    ...
                    type="submit"
                    className={` ml-auto py-2 px-8  text-white rounded-xl  ${IsDisabled.saveBtn ? `bg-gray-400 cursor-not-allowed ` : `bg-black hover:text-black hover:bg-white hover:border border-black cursor-pointer  `}  `}
                    onClick={(e) => {
                      e.preventDefault();
          ...
          {isFeedBack && (
            <div className= {` ${feedbackText?.endsWith("Successfully") ? `bg-green-500` : `bg-red-500` }   border w-[80%] h-fit py-10 flex flex-col gap-4 justify-center items-center rounded-2xl absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2`} >
        ...
        <section className=" flex flex-col mb-10 md:mb-0  gap-7 px-2 overflow-y-scroll h-full w-full md:w-[38%] ">

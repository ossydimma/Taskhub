    ...
    <div className="px-10 pt-7 pb-20 md:pb-0 w-full h-auto md:h-full flex flex-col gap-3 items-center">
        ...
        <div className="relative w-[60%] xxs:w-[42%] xs:w-[30%] sm:w-[36%] md:w-[28%] lmd:w-[25%] xl:w-[20%]  h-[25vh] sm:h-[33vh] md:h-[37vh] border-2 border-gray-300 rounded-full shadow-lg">
            ...
            {users.imageSrc ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-4xl font text-gray-500">NU</div>
            )}
            ...
            <div>
                <input type="file" accept="image/*" onChange={(e) => {
                    if (e.target.files && e.target.files[0]) {
                        ...
                        reader.onload = (event) => {
                            setUsers({...users, imageSrc : event.target?.result as string});
                ...
                <label htmlFor="fileInput" className="absolute bottom-4 right-4 flex items-center justify-center cursor-pointer stroke-white">
                    <svg
        ...
        <section className=" flex flex-col gap-4 md:flex-row md:gap-8 w-full justify-between items-center pt-2 pb-4 border-b-2 border-gray-400 border-dashed">
            ...
            <div className=" w-full md:w-[calc(1/4-1rem)] text-white text-center py-4 cursor-pointer bg-black rounded-2xl border">
                <h2 className="font-extrabold md:text=[0.85rem] lmd:text-[1rem]">Completed Tasks</h2>
                <p className="text-sm font-bold ">1000</p>
            </div>
            <div className=" w-full md:w-[calc(1/4-1rem)] text-white text-center py-4 cursor-pointer bg-black rounded-2xl border">
                <h2 className="font-extrabold md:text=[0.85rem] lmd:text-[1rem]">Pending Tasks</h2>
                <p className="text-sm font-bold ">1000</p>
            </div>
            <Link href={"/documentation"} className=" w-full md:w-[calc(1/4-1rem)] text-white text-center py-4 cursor-pointer bg-black rounded-2xl border">
